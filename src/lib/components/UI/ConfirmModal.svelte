<script lang="ts">
  ...
  export let body: string = 'Are you sure you want to delete this item?';
  export let confirmText: string = "Yes, I'm sure";
  export let cancelText: string = 'No, cancel';
...
<div
  tabindex="-1"
  aria-hidden="true"
  class="fixed top-0 left-0 right-0 z-50 flex items-center justify-center w-full overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full backdrop:bg-gray-900/60 bg-gray-900/60">
  <div class="relative w-full h-full max-w-md p-4 md:h-auto">
    ...
    <div class="relative p-4 text-center bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
      <button
        ...
        data-modal-toggle="deleteModal" on:click={closeModal}>
        <svg
          aria-hidden="true"
          class="w-5 h-5"
          fill="currentColor"
      ...
      <h3 class="mb-4 text-lg font-normal text-gray-500 dark:text-gray-400">{title}</h3>
